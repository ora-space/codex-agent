...
import {
  CodexClient,
  type SpawnedProcess,
} from "../src/services/codex-client.ts";
import { decodeLines } from "../src/services/ndjson.ts";

Deno.test("ora-space.codex declares an agent kind", async () => {
...
// proves discovery picks the `model` category out specifically rather than reporting whichever
// option happens to come first.
Deno.test("models come from the probe session's model config option", async () => {
  ...
  assertEquals(models, [
    ...
    { id: "o4-mini", displayName: "o4-mini", default: false },
  ]);
});
...
Deno.test("discovery without a workspace is refused rather than guessed", () => {
  const error = (() => {
    try {
      listCodexModels(fakeProcesses(defaultAcpAgent), "");
      return undefined;
    } catch (thrown) {
...
Deno.test("the Skill Resource is declared in the shape Ora materializes", () => {
  ...
  assertEquals(SKILLS_RESOURCE.workspaceRelativePath, ".agents/skills");
...
/**
 * Starts a `CodexClient` over a scripted adapter, recording every frame written to it.
 *
...
async function runningClient(
  spawns: number[] = [],
): Promise<{ client: CodexClient; written: JsonValue[] }> {
