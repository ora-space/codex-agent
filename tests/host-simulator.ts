/**
 * Drives the installed plugin exactly the way Ora's host does, against a real Codex ACP adapter.
 *
...
const effectSurfaces = registration.effectSurfaces ?? [];
check(effectSurfaces.length > 0, "registration declares an Effect surface");
...
console.log(
  `ok: session/new ${
    JSON.stringify(sessionResult ?? acpFrame(session).error).slice(0, 200)
  }`,
);
...
check(
  ...
  "agent/listModels is empty because Codex has no pre-session model list",
);
console.log("ok: listModels [] (Codex publishes models through ACP only)");
...
check(
  idleState === "ready",
  "waitForIdle reports ready with no turn in flight",
);
...
check(restarted.error === undefined, "effect/restart succeeds");
console.log("ok: effect/restart (adapter respawned)");

// The barrier must release a fully working bridge: prove the respawned adapter still answers ACP.
await send({
  jsonrpc: "2.0",
  method: "agent/acp",
  params: {
...
check(
  typeof (acpFrame(sessionAfterRestart).result as
    | { sessionId?: string }
    | undefined)
    ?.sessionId === "string",
  "session/new after restart returns an id",
);
console.log(
