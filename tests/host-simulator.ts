/**
 * Drives the plugin exactly the way Ora's host does, against a real Codex ACP adapter.
 *
...
async function waitFor(
  ...
  while (true) {
    ...
    if (isChildProcessRequest(message)) {
      // The plugin no longer spawns `codex-acp` itself; it asks this simulator, playing the host,
 ...
 * the real `codex-acp` process and relays its stdout/stderr/exit back as notifications.
 */
...
// second, one-shot `codex-acp` through this simulator rather than by borrowing the connection
