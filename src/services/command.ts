...
/**
 * Resolves the commands that can launch Codex's ACP adapter, in priority order.
 *
 * `ORA_CODEX_ACP_BIN` pins one exact binary and is returned alone: a pin that quietly fell back to
 * whatever is on PATH would run a different adapter than the user asked for, which is worse than
 * failing. Without a pin, Windows gets both spellings because npm installs only a
 * `codex-acp.cmd` shim while a bun or standalone install exposes the bare name.
 */
export function resolveCodexCommands(): string[] {
  const explicit = readEnv("ORA_CODEX_ACP_BIN")?.trim();
...
export async function tryEachCandidate<T>(
  ...
  throw new PluginMethodError(
    ...
    `Codex's ACP adapter is not installed or not on PATH (tried: ${
      candidates.join(", ")
    }); install it with \`npm i -g @agentclientprotocol/codex-acp\` or set ORA_CODEX_ACP_BIN`,
  );
}
