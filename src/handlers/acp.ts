 ...
 * already agreed on. `effects` only reads `method` and `id` off the envelope, to hold a new turn
 * behind the Skill Effect barrier when one is engaged; see {@link SkillEffectCoordinator}.
 *
...
export function forwardAcpFrame(
  ...
  if (!client.running) {
    log.warn("dropping host ACP frame: the Codex agent is not running", {
      context: summarize(frame),
    });
    return;
  }
