...
import { spawnCodex } from "../services/command.ts";

/** The ACP revision this plugin speaks; the same one Ora declares on its own connection. */
const ACP_PROTOCOL_VERSION = 1;

/**
 * How long one workspace's answer is reused before Codex is asked again.
 *
 ...
 * without outliving the reason a catalog changes — logging into a provider, or editing Codex's
...
export function invalidateCodexModels(cwd: string): void {
  catalogs.delete(cwd);
}

/** Drops every workspace catalog when this plugin generation is going away. */
export function invalidateAllCodexModels(): void {
  catalogs.clear();
}

/**
 * Serves `agent/list_models` for one workspace by asking Codex's adapter itself.
 *
 * `codex-acp` publishes its model (and reasoning-effort) choices in one place only: the
 * `configOptions` an ACP session carries, as `category: "model"` / `category: "reasoning-effort"`
 * entries. There is no pre-session listing to read, so discovery has to hold a whole ACP
 * conversation, which is why it needs its own process rather than the connection Ora is reading;
 * see {@link AcpProbe}. Those same options still reach Ora unchanged from the live session, so the
 * pre-session list this builds and the in-session picker are two readings of one source rather than
 * two sources that can disagree. Only the `model` category feeds the pre-session picker;
 * reasoning-effort is a session-level choice with nothing to show before one exists.
 *
...
/**
 * Runs one throwaway `codex-acp` and reads the model selector out of a session it creates.
 *
